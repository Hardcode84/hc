...
#define DEBUG_TYPE "hc-execution-engine"
...
class hc::ExecutionEngine::SimpleObjectCache : public llvm::ObjectCache {
public:
...
llvm::Expected<hc::ExecutionEngine::ModuleHandle>
hc::ExecutionEngine::loadModule(mlir::ModuleOp m) {
  assert(m);
...
void hc::ExecutionEngine::releaseModule(ModuleHandle handle) {
  assert(handle);
...
llvm::Expected<void *>
hc::ExecutionEngine::lookup(hc::ExecutionEngine::ModuleHandle handle,
...
void hc::ExecutionEngine::dumpToObjectFile(llvm::StringRef filename) {
  if (cache == nullptr) {
