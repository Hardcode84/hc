...
//       CHECK:     %[[V11:.*]] = hkernel.subview %[[V8]] : !hkernel<buffer <("W" - "$GROUP_ID0" * "$GROUP_SHAPE0") x ("H" - "$GROUP_ID1" * "$GROUP_SHAPE1")> x ![[LIT]]>[%[[V9]], %[[V10]]] : !hkernel<slice ![[SYM4]] : none : none>, !hkernel<slice ![[EXPR3]] : none : none> -> !hkernel<buffer <(-"$LOCAL_ID0" - "$GROUP_ID0" * "$GROUP_SHAPE0" + "W") x (-("$GROUP_ID1" * "$GROUP_SHAPE1") + "H" - ("$LOCAL_ID1" floordiv "$SUBGROUP_SIZE") * "$SUBGROUP_SIZE")> x ![[LIT]]>
//       CHECK:     %[[V12:.*]] = hkernel.load %[[V11]] : !hkernel<buffer <(-"$LOCAL_ID0" - "$GROUP_ID0" * "$GROUP_SHAPE0" + "W") x (-("$GROUP_ID1" * "$GROUP_SHAPE1") + "H" - ("$LOCAL_ID1" floordiv "$SUBGROUP_SIZE") * "$SUBGROUP_SIZE")> x ![[LIT]]>[%[[V1]], %[[V0]]] : ![[LIT1]], ![[SYM6]] -> !hkernel<tensor <1 x "$SUBGROUP_SIZE"> x ![[LIT]]>
...
//       CHECK:     %[[V16:.*]] = hkernel.subview %[[V13]] : !hkernel<buffer <("W" - "$GROUP_ID0" * "$GROUP_SHAPE0") x ("H" - "$GROUP_ID1" * "$GROUP_SHAPE1")> x ![[LIT]]>[%[[V14]], %[[V15]]] : !hkernel<slice ![[SYM4]] : none : none>, !hkernel<slice ![[EXPR3]] : none : none> -> !hkernel<buffer <(-"$LOCAL_ID0" - "$GROUP_ID0" * "$GROUP_SHAPE0" + "W") x (-("$GROUP_ID1" * "$GROUP_SHAPE1") + "H" - ("$LOCAL_ID1" floordiv "$SUBGROUP_SIZE") * "$SUBGROUP_SIZE")> x ![[LIT]]>
//       CHECK:     hkernel.store %[[V16]] : !hkernel<buffer <(-"$LOCAL_ID0" - "$GROUP_ID0" * "$GROUP_SHAPE0" + "W") x (-("$GROUP_ID1" * "$GROUP_SHAPE1") + "H" - ("$LOCAL_ID1" floordiv "$SUBGROUP_SIZE") * "$SUBGROUP_SIZE")> x ![[LIT]]> = %[[V12]] : !hkernel<tensor <1 x "$SUBGROUP_SIZE"> x ![[LIT]]>
//       CHECK:   }
//       CHECK:   return
//       CHECK: }
