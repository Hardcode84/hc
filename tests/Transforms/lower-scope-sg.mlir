...
//       CHECK:     %[[R:.*]] = hkernel.load %{{.*}} : !hkernel<buffer <(-("$GROUP_ID0" * "$GROUP_SHAPE0") - "$LOCAL_ID0" + "W") x (-("$GROUP_ID1" * "$GROUP_SHAPE1") - "$LOCAL_ID1" - "$LOCAL_ID1" floordiv "$SUBGROUP_SIZE" + "H")> x ![[LIT0]]>[%[[V0]], %[[V0]]] : ![[LIT1]], ![[LIT1]] -> !hkernel<tensor <1 x 1> x ![[LIT0]]>
//       CHECK:     hkernel.store %{{.*}} : !hkernel<buffer <(-("$GROUP_ID0" * "$GROUP_SHAPE0") - "$LOCAL_ID0" + "W") x (-("$GROUP_ID1" * "$GROUP_SHAPE1") - "$LOCAL_ID1" - "$LOCAL_ID1" floordiv "$SUBGROUP_SIZE" + "H")> x ![[LIT0]]> = %[[R]] : !hkernel<tensor <1 x 1> x ![[LIT0]]>
